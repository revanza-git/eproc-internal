...
$(function(){
				...
				render : function(el, data){
					...
					$.ajax({
						...
						success: function(dpt){
							...
							$('#tab2 .tab-content').append('<fieldset class="form-group form0 " for=""><label for="">Usulan Non DPT</label><input type="text" class="form-control" name="usulan" required></fieldset>');
		...
		renderContent: function(el, value, key){
			...
			// DATA PEMAKETAN PENGADAAN DALAM STATUS FPPBJ
				...
				// FPPBJ waiting to approve by manager
					...
					status = 'FPPBJ (ditolak)<span class="tooltiptext reject">'+keterangan+'</span>';
					badge = 'danger fppbj_reject tooltip';
					// reject_notif(keterangan);
				}
				else if (is_status == "0" && is_approve == "1" && tipe_pengadaan == 'barang' && _persetujuan == null && id_perencanaan_umum < 1 && is_reject == 1 && is_writeoff == 0) {
					status = 'FPPBJ (ditolak)<span class="tooltiptext reject">'+keterangan+'</span>';
					badge = 'danger fppbj_reject tooltip';
					// reject_notif(keterangan);
				}
				...
				// FPPBJ REJECT
				}else if(is_status == "0" && is_approve == "1" && is_reject == '1') {
			...
			// PENOLAKAN
					...
					status = 'FPPBJ (Di tolak HSSE)<span class="tooltiptext reject">'+keterangan+'</span>';
					badge = 'danger fppbj_reject tooltip';
					// reject_notif(keterangan);
	...
	var resiko = $('.buttonStep').modal({
		...
		render : function(el, data){
			...
			$('#step2 .form4').append('<input style="margin-left: 30%;" type="checkbox" value="1" name="is_multiyear">Multiyear Budget');
