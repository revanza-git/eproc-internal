...
class Riwayat extends MY_Controller
{
	...
	function pengadaan($id)
	{
		...
		$this_approval = $this->$modelAlias->getRiwayatPengadaan($id, 'approval');
		...
		if (count($this_approval) > 0) {
			foreach ($this_approval as $key => $value) {
				$_status = array(
					0 => 'FPPBJ',
					2 => 'FKPBJ',
					1 => 'FP3'
				...
				if (($value['date_approval'] == '' || $value['date_approval'] == null || $value['date_approval'] == '0000-00-00')) {
					$date_approval = $date_approval = date('d M Y', strtotime($value['entry_stamp']));;
				} else {
