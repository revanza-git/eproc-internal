...
class Main extends CI_Controller {
	...
	public function index(){
		if($this->session->userdata('user')){
			header("Location: http://local.eproc.intra.com/eproc_pengadaan/dashboard");
		}elseif($this->session->userdata('admin')){
			if ($this->session->userdata('admin')['app_type'] == 1) {
				header("Location: http://local.eproc.intra.com/eproc_pengadaan/admin");
			}else{
				redirect('dashboard');				
			}
		}else{
		...
		if($this->input->post('username') && $this->input->post('password')){
			...
			if($is_logged){
				...
				if($this->session->userdata('user')){
					...
					header("Location:http://local.eproc.vms.com/eproc_pengadaan/main/login_user/".$name."/".$id_user."/".$id_sbu."/".$vendor_status."/".$is_active."/".$type."/".$app);
					...
					if ($this->session->userdata('admin')['app_type'] == 1) {
						...
						header("Location:http://local.eproc.intra.com/eproc_pengadaan/main/login_admin/".$id_user."/".$name."/".$id_role."/".$role_name."/".$type."/".$app."/".$id_sbu."/".$sbu_name);
					}else{
						redirect('dashboard');				
					}
				}
